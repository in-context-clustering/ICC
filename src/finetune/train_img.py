...
from src.utils import get_vision_chat_message, compute_clustering_accuracy, postprocess, extract_cluster_info, move_to_cuda
...
class CustomArguments:
    patch_size: Optional[int] = field(
        default=14,
        metadata={"help": "patch size after pooling"}
    )
...
class LLMSampleCB(WandbCallback):
    def __init__(self, trainer, test_dataset, processor, output_dir, max_new_tokens=10000, log_model=None):
        ...
        for ex in test_dataset:
            ...
            inputs['pixel_values'] = self.processor.image_processor(ex["images"], return_tensors="pt")['pixel_values']
            num_clusters, condition = extract_cluster_info(ex["conversations"][0])
            info = {'clusters': num_clusters, 'condition': condition}
    ...
    def samples_table(self, step):
        ...
        for ex in tqdm(self.preprocessed_dataset):
            ...
            avg_acc[f"{ex['info']['clusters']}clusters"].append(acc)
            avg_acc[f"{ex['info']['condition']}"].append(acc)
        return {f"eval/acc-{k}": sum(v)/len(v) for k,v in avg_acc.items()}
...
def train():
    ...
    model_args, data_args, training_args, lora_args, custom_args = parser.parse_args_into_dataclasses()
